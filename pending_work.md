...
**Mockups for this app** (all in `../shared-docs/design-mockups/`):
  - `sibling-family-crypto-signal.html — Home / Markets`
  - `sibling-family-crypto-signal-SIGNALS.html — Signal detail`
  - `sibling-family-crypto-signal-BACKTESTER.html — Backtester`
  - `sibling-family-crypto-signal-REGIMES.html — Regimes`

**Family:** sibling
**Accent:** #22d36f (signal-green)
**Priority note:** Pilot — prove the design system here first.

### Redesign tasks — work in order, commit after each

- [ ] 1. Copy `common/ui_design_system.py` → `ui/design_system.py`. Import in `app.py`. Call `inject_theme("crypto-signal-app")` at the top of every page (after `set_page_config`, before `apply_theme`).
